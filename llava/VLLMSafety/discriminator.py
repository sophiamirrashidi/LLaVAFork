...
class Discriminator(nn.Module):
    ...
    def forward(self, data, d_mode):
        ...
        total_lang_preds = 0
        img_correct_count = torch.eq(torch.ge(img_pred, 0.5).float(), torch.ones_like(img_pred)).sum().item()  # Compare prediction against true class before smoothing
        ...
        for lang_tensor in data["lang"]:
            ...
            # Accuracy calculations, comparing against true class before smoothing
            lang_correct = torch.eq(torch.lt(lang_pred, 0.5).float(), torch.ones_like(lang_pred)).sum().item()
